...
int main(int argc, char *argv[])
{
  ...
  reg_create();
  ref = zVecAlloc( DIM );
  if( argc > 1 ) zVecSetElem( ((dzLin *)sys._prm)->x, 0, atof(argv[1]) );
  for( i=0; i<STEP; i++ ){
    printf( "%f %f %f\n", input, zVecElem(ref,0), dzSysOutputVal(&sys,0) );
    input = dzLinStateFeedback( sys._prm, ref, opt_gain );
    dzSysUpdate( &sys, DT );
  }
  zVecFree( opt_gain );
  dzSysDestroy( &sys );
