 ...
 *
 * dzSysCreateLin() creates a linear system \a sys.
 ...
 *
 * dzSysCreateLin() assigns \a lin to \a sys, and not newly
 * allocate particular work space for it.
 * Memory for \a lin has to be independently managed.
 * \return
 * dzSysCreateLin() returns the null pointer if \a dt is too
 * short or negative. Otherwise, a pointer \a sys is returned.
 */
__EXPORT dzSys *dzSysCreateLin(dzSys *sys, dzLin *lin);
